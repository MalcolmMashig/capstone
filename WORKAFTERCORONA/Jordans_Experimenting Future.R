...
here::here(
  'WORKAFTERCORONA', 'Jordans_Experimenting.R'
) %>% 
  source()
...
future <- future %>% 
  mutate(
    ...
    predicted_kbb1 = kbb_submodel %>% predict(newdata = future)
  )
...
future <- future %>% 
  mutate(
    ...
    predicted_kbb2 = kbb_submodel %>% predict(newdata = future)
  )
...
future <- future %>% 
  mutate(
    ...
    predicted_kbb3 = kbb_submodel %>% predict(newdata = future)
  )
...
predictions <- fangraphs_clean %>% 
  filter(Season == 2019) %>% 
  select(Name, Team, lag_xfip, Age, FBv, FBP, xFIP, `K/BB`) %>% 
  right_join(predictions)
